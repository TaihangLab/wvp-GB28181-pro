  ...
  methods: {
    ...
    batchImport() {
      ...
      this.$confirm(`确认删除选中的 ${this.selectedSkills.length} 个技能吗？此操作不可恢复！`, '批量删除', {
        ...
        // 保存删除数量
        const deleteCount = this.selectedSkills.length
        ...
        // 更新总数
        this.totalCount = this.filteredSkills.length
        ...
        this.$message.success(`已成功删除 ${deleteCount} 个技能`)
      }).catch(() => {
    ...
    // 卡片操作
    editSkill(skill) {
      this.$message.info(`编辑技能: ${skill.name}`)
    },

    toggleSkillStatus(skill) {
      skill.status = skill.status === 'online' ? 'offline' : 'online'
      this.$message.success(`${skill.name} 已${skill.status === 'online' ? '上线' : '下线'}`)
    },
