 ...
 * 拉流代理截图相关API
 */
@Tag(name = "拉流代理截图", description = "")
@RestController
@Slf4j
@RequestMapping(value = "/api/proxy")
public class StreamProxySnapshotController {
     ...
     * 获取截图
     */
    @Operation(summary = "获取拉流代理截图", security = @SecurityRequirement(name = JwtUtils.HEADER))
    @Parameter(name = "app", description = "应用名", required = true)
    @Parameter(name = "stream", description = "流ID", required = true)
    @GetMapping("/snap")
    public DeferredResult<String> getSnap(String app, String stream) {
        if (log.isDebugEnabled()) {
            log.debug("获取拉流代理截图: {}/{}", app, stream);
        }

        DeferredResult<String> result = new DeferredResult<>(3 * 1000L);
        String key = DeferredResultHolder.CALLBACK_CMD_PROXY_SNAP + app + "_" + stream;
        ...
        if (streamProxy == null) {
            message.setData(WVPResult.fail(ErrorCode.ERROR404.getCode(), "拉流代理未找到"));
